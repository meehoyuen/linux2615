...
#include <scsi/scsi_transport_spi2.h>
...
#define spi2_dv_pending(x) (((struct spi2_transport_attrs *)&(x)->starget_data)->dv_pending)
#define spi2_dv_sema(x) (((struct spi2_transport_attrs *)&(x)->starget_data)->dv_sema)

struct spi2_internal {
	struct scsi_transport_template t;
	struct spi2_function_template *f;
...
#define to_spi_internal(tmpl)	container_of(tmpl, struct spi2_internal, t)
				     ...
				     struct class_device *cdev)
{
	...
	spi2_dv_pending(starget) = 0;
	spi_initial_dv(starget) = 0;
	init_MUTEX(&spi2_dv_sema(starget));
...
{									\
	...
	struct spi2_transport_attrs *tp;					\
									\
	tp = (struct spi2_transport_attrs *)&starget->starget_data;	\
...
{									\
	...
	struct spi2_transport_attrs *tp;					\
	struct spi2_internal *i = to_spi_internal(shost->transportt);	\
	tp = (struct spi2_transport_attrs *)&starget->starget_data;	\
	if (i->f->get_##field)						\
...
{									\
	...
	struct spi2_internal *i = to_spi_internal(shost->transportt);	\
	struct spi2_transport_attrs *tp					\
		= (struct spi2_transport_attrs *)&starget->starget_data;	\
									\
	val = simple_strtoul(buf, NULL, 0);				\
...
spi2_dv_device_echo_buffer(struct scsi_device *sdev, u8 *buffer,
...
static enum spi_compare_returns
spi2_dv_device_compare_inquiry(struct scsi_device *sdev, u8 *buffer,
			      u8 *ptr, const int retries)
{
	int r, result;
...
static enum spi_compare_returns
spi2_dv_retrain(struct scsi_device *sdev, u8 *buffer, u8 *ptr,
	       enum spi_compare_returns 
	       (*compare_fn)(struct scsi_device *, u8 *, u8 *, int))
{
	struct spi2_internal *i = to_spi_internal(sdev->host->transportt);
	struct scsi_target *starget = sdev->sdev_target;
...
static int
spi2_dv_device_get_echo_buffer(struct scsi_device *sdev, u8 *buffer)
{
	int l, result;
...
static void
spi2_dv_device_internal(struct scsi_device *sdev, u8 *buffer)
{
	struct spi2_internal *i = to_spi_internal(sdev->host->transportt);
	struct scsi_target *starget = sdev->sdev_target;
	...
	if (spi2_dv_device_compare_inquiry(sdev, buffer, buffer, DV_LOOPS)
	    != SPI_COMPARE_SUCCESS) {
	...
	if (i->f->set_width && spi_max_width(starget) &&
		...
		if (spi2_dv_device_compare_inquiry(sdev, buffer,
						   buffer + len,
						   DV_LOOPS)
		    != SPI_COMPARE_SUCCESS) {
 ...
 retry:
	...
	spi2_dv_retrain(sdev, buffer, buffer + sdev->inquiry_len,
		       spi2_dv_device_compare_inquiry);
	...
	if (len == -1 && spi_dt(starget))
		len = spi2_dv_device_get_echo_buffer(sdev, buffer);
	...
	if (spi2_dv_retrain(sdev, buffer, buffer + len,
			   spi2_dv_device_echo_buffer)
	    == SPI_COMPARE_SKIP_TEST) {
...
/**	spi2_dv_device - Do Domain Validation on the device
 ...
 */
void
spi2_dv_device(struct scsi_device *sdev)
{
	struct scsi_target *starget = sdev->sdev_target;
	u8 *buffer;
	...
	spi2_dv_pending(starget) = 1;
	down(&spi2_dv_sema(starget));

	starget_printk(KERN_INFO, starget, "Beginning Domain Validation\n");

	spi2_dv_device_internal(sdev, buffer);

	starget_printk(KERN_INFO, starget, "Ending Domain Validation\n");

	up(&spi2_dv_sema(starget));
	spi2_dv_pending(starget) = 0;
...
static void
spi2_dv_device_work_wrapper(void *data)
{
	...
	kfree(wqw);
	spi2_dv_device(sdev);
	spi2_dv_pending(sdev->sdev_target) = 0;
	scsi_device_put(sdev);
}


/**
 *	spi2_schedule_dv_device - schedule domain validation to occur on the device
 *	@sdev:	The device to validate
 *
 *	Identical to spi2_dv_device() above, except that the DV will be
 *	scheduled to occur in a workqueue later.  All memory allocations
 *	are atomic, so may be called from any context including those holding
 *	SCSI locks.
 */
void
spi2_schedule_dv_device(struct scsi_device *sdev)
{
	...
	if (unlikely(spi2_dv_pending(sdev->sdev_target))) {
		kfree(wqw);
		return;
	}
	/* Set pending early (dv_device doesn't check it, only sets it) */
	spi2_dv_pending(sdev->sdev_target) = 1;
	if (unlikely(scsi_device_get(sdev))) {
		kfree(wqw);
		spi2_dv_pending(sdev->sdev_target) = 0;
		return;
	...
	INIT_WORK(&wqw->work, spi2_dv_device_work_wrapper, wqw);
	wqw->sdev = sdev;

	schedule_work(&wqw->work);
}
EXPORT_SYMBOL(spi2_schedule_dv_device);

/**
 * spi2_display_xfer_agreement - Print the current target transfer agreement
 ...
 */
void spi2_display_xfer_agreement(struct scsi_target *starget)
{
	struct spi2_transport_attrs *tp;
	tp = (struct spi2_transport_attrs *)&starget->starget_data;
...
EXPORT_SYMBOL(spi2_display_xfer_agreement);

int spi2_populate_width_msg(unsigned char *msg, int width)
{
...
EXPORT_SYMBOL_GPL(spi2_populate_width_msg);

int spi2_populate_sync_msg(unsigned char *msg, int period, int offset)
{
	msg[0] = EXTENDED_MESSAGE;
	msg[1] = 3;
	msg[2] = EXTENDED_SDTR;
...
EXPORT_SYMBOL_GPL(spi2_populate_sync_msg);

int spi2_populate_ppr_msg(unsigned char *msg, int period, int offset,
...
EXPORT_SYMBOL_GPL(spi2_populate_ppr_msg);
...
int spi2_print_msg(const unsigned char *msg)
{
	int len = 1, i;
...
spi2_attach_transport(struct spi2_function_template *ft)
{
	int count = 0;
	struct spi2_internal *i = kzalloc(sizeof(struct spi2_internal),
					 GFP_KERNEL);
	...
	i->t.target_size = sizeof(struct spi2_transport_attrs);
	i->t.host_attrs.ac.class = &spi_host_class.class;
	i->t.host_attrs.ac.attrs = &i->host_attrs[0];
	i->t.host_attrs.ac.match = spi_host_match;
	transport_container_register(&i->t.host_attrs);
	i->t.host_size = sizeof(struct spi2_host_attrs);
	i->f = ft;
...
EXPORT_SYMBOL(spi2_attach_transport);

void spi2_release_transport(struct scsi_transport_template *t)
{
	struct spi2_internal *i = to_spi_internal(t);
...
EXPORT_SYMBOL(spi2_release_transport);
