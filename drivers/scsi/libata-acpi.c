...
void libata_acpi_notify(acpi_handle device, u32 type, void *data) {
...
static int libata_acpi_find_device(struct device *dev, acpi_handle *handle)
{
	return -ENODEV;
}

static int libata_acpi_find_channel(struct device *dev, acpi_handle *handle)
{
	long int i;
	
	if (sscanf(dev->bus_id, "host%lu", &i) != 1)
		return -ENODEV;

	*handle = acpi_get_child(DEVICE_ACPI_HANDLE(dev->parent), 
				 (acpi_integer) i);

	if (!*handle)
		return -ENODEV;

	acpi_install_notify_handler(*handle, ACPI_ALL_NOTIFY, 
				    &libata_acpi_notify, (void *)i);
	
	return 0;
}

static struct acpi_bus_type libata_acpi_bus = {
	.bus = &scsi_bus_type,
	.find_bridge = libata_acpi_find_channel,
	.find_device = libata_acpi_find_device,
};

void ata_acpi_bus_register()
{
	register_acpi_bus_type(&libata_acpi_bus);
}
EXPORT_SYMBOL_GPL(ata_acpi_bus_register);
