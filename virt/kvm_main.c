...
static void kvm_assigned_dev_interrupt_work_handler(struct work_struct *work)
{
    ...
    assigned_dev = container_of(work, struct kvm_assigned_dev_kernel, interrupt_work);

    /* This is taken to safely inject irq inside the guest. When
     * the interrupt injection (or the ioapic code) uses a
     * finer-grained lock, update this
     */
    down(&assigned_dev->kvm->lock);
    kvm_set_irq(assigned_dev->kvm, assigned_dev->irq_source_id, assigned_dev->guest_irq, 1);

    if (assigned_dev->irq_requested_type & KVM_ASSIGNED_DEV_GUEST_MSI) {
        enable_irq(assigned_dev->host_irq);
        assigned_dev->host_irq_disabled = false;
    }
    up(&assigned_dev->kvm->lock);
}
...
static void kvm_assigned_dev_ack_irq(struct kvm_irq_ack_notifier *kian)
{
    ...
    dev = container_of(kian, struct kvm_assigned_dev_kernel, ack_notifier);
                   ...
                   *assigned_irq)
{
    ...
    if (!match->irq_requested_type) {
        INIT_WORK(&match->interrupt_work, kvm_assigned_dev_interrupt_work_handler, &match->interrupt_work);
        if (irqchip_in_kernel(kvm)) {
...
static int kvm_vcpu_nopage(struct vm_area_struct *vma, unsigned long address, int *type)
{
...
static int kvm_vm_nopage(struct vm_area_struct *vma, unsigned long address, int *type)
{
    struct page *page[1];
    unsigned long addr;
    int npages;
          ...
          struct module *module)
{
    ...
    kvm_vcpu_cache = kmem_cache_create("kvm_vcpu", vcpu_size, __alignof__(struct kvm_vcpu), 0, NULL, NULL);
    if (!kvm_vcpu_cache) {
        r = -ENOMEM;
        goto out_free_5;
    }
