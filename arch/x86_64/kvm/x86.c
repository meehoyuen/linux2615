...
static void kvm_write_guest_time(struct kvm_vcpu *v)
{
    ...
    //vcpu->hv_clock.system_time = ts.tv_nsec + (NSEC_PER_SEC * (u64)ts.tv_sec);
    /*
...
long kvm_arch_vcpu_ioctl(struct file *filp, unsigned int ioctl, unsigned long arg)
{
    ...
    void __user *argp = (void __user *)arg;
    int r;
    struct kvm_lapic_state *lapic_state = NULL;

    switch (ioctl) {
    case KVM_GET_LAPIC: {
        lapic_state = kzalloc(sizeof(struct kvm_lapic_state), GFP_KERNEL);
        ...
        if (r)
            goto out;
        r = -EFAULT;
        if (copy_to_user(argp, lapic_state, sizeof(struct kvm_lapic_state)))
            goto out;
        r = 0;
        break;
    }
    case KVM_SET_LAPIC: {
        lapic_state = kmalloc(sizeof(struct kvm_lapic_state), GFP_KERNEL);
        r = -ENOMEM;
        if (!lapic_state)
            goto out;
        r = -EFAULT;
        if (copy_from_user(lapic_state, argp, sizeof(struct kvm_lapic_state)))
            goto out;
